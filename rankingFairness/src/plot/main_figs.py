...
def plot_synthetic(EO_constraint, cost_groups, total_cost, rankingAlgos, rankingAlgos_dict, 
    SynthMarker_dict, save_path, filename, fig_kw, dcg_option=False, legendFileName=None, dcg=None):
    ...
    if dcg_option:
        gs1 = gridspec.GridSpecFromSubplotSpec(2, 7, subplot_spec=grid[0])
        ax1 = fig.add_subplot(gs1[:,:2]) #EOR plot
        ax31 = fig.add_subplot(gs1[:1,6:]) # total costs
        ax32 = fig.add_subplot(gs1[1:,6:]) # ndcg
        ...
        ax14 = fig.add_subplot(gs1[:1,5:6], sharey = ax11)
        ax21 = fig.add_subplot(gs1[1:,2:3])
        ax22 = fig.add_subplot(gs1[1:,3:4], sharey = ax21)
        ax23 = fig.add_subplot(gs1[1:,4:5], sharey = ax21)
        group_costs_axs = [ax11, ax12, ax13, ax14, ax21, ax22, ax23]

    else: 
        gs1 = gridspec.GridSpecFromSubplotSpec(4, 16, subplot_spec=grid[0])
        ax1 = fig.add_subplot(gs1[:,:4]) #EOR plot
        ax3 = fig.add_subplot(gs1[:,12:]) #total costs plot
    ...
    for g in range(num_groups):
        ...
        group_costs_axs[4].plot(np.arange(num_docs)+1, cost_groups[rankingAlgos_dict.get('EXP'),g,...], linestyle='dashed',
         marker=MARKERS[g], markeredgecolor=COLORMAP['EXP'], markeredgewidth=markeredgewidth,
        ...
        group_costs_axs[5].plot(np.arange(num_docs)+1, cost_groups[rankingAlgos_dict.get('RA'),g,...], linestyle='dashed',
         marker=MARKERS[g], markeredgecolor=COLORMAP['RA'], markeredgewidth=markeredgewidth,
        ...
        group_costs_axs[6].plot(np.arange(num_docs)+1, cost_groups[rankingAlgos_dict.get('FS'),g,...], linestyle='dashed',
    ...
    group_costs_axs[4].legend(handles=[Line2D([], [], color=COLORMAP['EXP'], linestyle='solid', 
                linewidth=linewidth, label='EXP')], fontsize=ticklabelsize, frameon=False, loc='upper right')
    group_costs_axs[5].legend(handles=[Line2D([], [], color=COLORMAP['RA'], linestyle='solid', 
                linewidth=linewidth, label='RA')], fontsize=ticklabelsize, frameon=False, loc='upper right')

    group_costs_axs[6].legend(handles=[Line2D([], [], color=COLORMAP['FS'], linestyle='solid', 
